package com.ogfiraffers.section01.servicemethod;
...
public class ServiceMethodTestServlet extends HttpServlet {
    ...
    * 따라서 ServletRequest라는 추상화 된 타입으로 사용하고 실제 인스턴스는 HttpServleRequest로 사용하여
    * 나중에 service method의 인자 타입은 변경하지 않고 다른 프로토콜을 사용하는 Request로 변경이 가능하다. (다형성) */
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
      ...
      * 실제 사용 시에는 다운 캐스팅해서 사용해야 한다. */
        ...
        String httpMethod = httpReqeust.getMethod();
        /* 요청 내용 출력 */
        System.out.println("httpMethod : " + httpMethod);

        if("GET".equals(httpMethod)) {
            /* GET 요청을 처리할 메소드로 요청과 응답 정보를 전달한다. */
            doGet(httpReqeust, httpResponse);
        } else {
            /* POST 요청을 처리할 메소드로 요청과 응답 정보를 전달한다. */
            doPost(httpReqeust, httpResponse);
        }
    }
    @Override
